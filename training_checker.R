...
# training.data <- read.csv("my_training.csv")
#  としても、同様の結果が得られる。
# 読み込めているかどうか確認

print(head(training.data)) # 先頭6行分のみ出力
browser()
...
print(head(training.data)) # 先頭6行分を再出力。解決。

training.data$date <- as.Date(training.data$date) # date型に変換しておく
browser()
...
plot(weight ~ date,data = subset.weight)
browser() # 一時停止

# 単回帰分析を行なってみる。 (y = ax + b [y: weight, x: date])

result.weight <- lm(weight ~ date,data = subset.weight)
print(result.weight)

# とりあえず分析結果をプロット

abline(result.weight) # 回帰式をプロット
browser() # 一時停止
...
result.weight.confidence <- predict(
    result.weight,
    interval="confidence"
    )
...
#     赤：信頼区間
#     青：予測区間
date.start = "2013/05/10"
date.last = "2013/07/04"

plot(
    x=subset.weight$date,
    y=subset.weight$weight,
...
abline(result.fat) # 回帰式をプロット

# 回帰分析結果の概要をチェック

print(summary(result.fat))

# 信頼区間を算出
result.fat.confidence <- predict(
    result.fat,
    interval="confidence"
    )

# 予測区間を算出
result.fat.predict <- predict(
    result.fat,
    interval="prediction"
    ) 
